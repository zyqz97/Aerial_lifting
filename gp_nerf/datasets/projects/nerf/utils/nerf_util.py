...
def sample_dists_zyq(ray_size, dist_range, intvs, stratified, device="cuda"):
    ...
    Args:
        ...
        intvs: (int): Number of points sampled on a ray.   # N
    ...
    """
    batch_size, num_rays = ray_size  # B, R
    dist_min, dist_max = dist_range  # near and far
    
    near, far = dist_min[0,:,:,0], dist_max[0,:,:,0]
    z_fg = torch.linspace(0, 1, intvs, device=device)
    z_vals_inbound = near * (1 - z_fg) + far * z_fg  #  (R, N)
    z_vals_inbound = perturb_z_vals(z_vals_inbound, intvs, 1, num_rays) #  (R, N)

    dists = z_vals_inbound.view(batch_size, num_rays, intvs, 1)  #  (1, R, N, 1)

    return dists

def perturb_z_vals(z_vals, samples, perturb, N_rays):
