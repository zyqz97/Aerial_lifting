...
class LoTDEncoding(nn.Module):
    def __init__(
        ...
        param_init_cfg={'method': 'uniform_to_type', 'bound': 1.0e-4}, 
        ...
        dtype=torch.float16, device=torch.device('cuda')) -> None:
        super().__init__()
        ...
        self.space = AABBSpace(bounding_size=bounding_size, aabb=aabb, device=self.device)  # 这里如果给定了aabb，则不做操作，另外记录原始的scale（aabb的长宽高的一半）
        
        #------- LoTD Metadata   # 下面对使用的dense+hash进行config定义，然后由LoTD初始化
        ...
        if lotd_auto_compute_cfg is not None:
            aspect_ratio = 1 if not lotd_use_cuboid else self.space.stretch.tolist() # 这里就是aabb的三个维度
        ...
        p = torch.zeros(self.lotd.n_params, device=self.device, dtype=torch.float)
    ...
    def inference_param(self):
        return self.flattened_params.to(self.dtype)
    ...
    def forward_dydx(self, input: torch.Tensor, max_level: int=None, need_loss_backward_input=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            ...
            need_loss_backward_input (bool, optional): Whether to propagate the gradient from the loss back to the input. Defaults to False.
        ...
        """
        output, dy_dx = self.lotd.forward_dydx(input/2.+0.5, self.flattened_params, max_level=(max_level or self.max_level), need_loss_backward_input=need_loss_backward_input)
    ...
    def backward_dydx(self, dL_dy: torch.Tensor, dy_dx: torch.Tensor, input: torch.Tensor, max_level: int=None, grad_guard=None) -> torch.Tensor:
        ...
        Args:
            ...
            dy_dx (torch.Tensor): [..., out_features*in_features], Pre-calculated dy_dx tensor by `forward_dydx`
            input (torch.Tensor): [..., in_features], Input relative position/coords in range [-1,1]
            max_level (int, optional): Maximum lotd level bypass. Defaults to None.
        ...
        """
        # NOTE: Need to multiply by 0.5, because `input` has already been divided by 2 during the `forward` process
        nablas = .5 * self.lotd.backward_dydx(dL_dy, dy_dx, input/2.+0.5, self.flattened_params, max_level=(max_level or self.max_level), grad_guard=grad_guard)
        return nablas
    ...
    def init_param_random(self):
        ...
        param_init_method = param_init_cfg['method']
...
if __name__ == "__main__":
    def test_ops(device=torch.device('cuda')):
        m = LoTDEncoding(
            ...
            param_init_cfg={'method': 'uniform_to_type', 'bound': 1.0e-4},  device=device, dtype=torch.float16)
        with torch.no_grad():
