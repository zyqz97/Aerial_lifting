...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        self.bound = 2 # opt.bound # bounding box half length, also used as the radius to random sample poses.
    ...
    def __getitem__(self, index):
        ...
        near = torch.zeros((self.num_rays,1), dtype=torch.int32)
        far = 2 * torch.ones((self.num_rays,1), dtype=torch.int32)
        rays = torch.cat([rays_o, rays_d, near, far], dim=1)

        item = {
                'rgbs': images,
                'rays': rays, 
                'img_indices': index * torch.ones(self.num_rays, dtype=torch.int32),
